...
exports.build = function (options) {
    ...
    fs.readFile(getInputHtmlFile(), "utf8",function(err,data){
        ...
        compressJsFiles(getScriptFiles(),function(){
            ...
            if(OUTPUT_CSS_FILE && OUTPUT_CSS_FILE !== ""){
                compressCssFiles(getCssFiles(),function(){
                    console.log('...done');
                    removeExistingCssTags();
                    writeOutNewHtml(true,true);
                });
            }
            else{
                writeOutNewHtml(true,false);
            }
...
function writeOutNewHtml(addjs,addCss){
    console.log('#6 Generating new html');
    if(addjs) $('body').append("\t<script src='" + jsfile + "'></script>");
    if(addCss) $('head').append("\t<link rel='stylesheet' href='" + cssfile + "'></link>");
    var op = $.html();
    tidy(op, {
        indent: true,
...
function parseOptions (options) {
    ...
    OUTPUT_CSS_FILE = options.cssBuildName;
}
